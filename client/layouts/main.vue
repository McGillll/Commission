<template>
              ...
              <div class="flex h-16 items-center justify-between px-4 sm:px-0">
                <div class="flex items-center">
                  <div class="shrink-0 flex items-center gap-2">
                    <img class="object-contain size-7" src="assets/img/vultify only.png" alt="Your Company" />
                    <button @click="navigateTo('/account')" class="text-white font-bold text-base">
                      Vaultify
                    </button>
                  </div>
                    ...
                    <Menu as="div" class="relative ml-3">
                      <div>
                        <MenuButton class="relative flex max-w-xs items-center rounded-full bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800">
                          ...
                          <UserCircleIcon class="size-7 text-gray-400 hover:text-white" aria-hidden="true"/>
                        </MenuButton>
                      </div>
          ...
          <DisclosurePanel class="border-b border-gray-700 md:hidden">
            ...
            <div class="border-t border-gray-700 pb-3 pt-4">
              <div class="flex items-center px-5">
                ...
                <div class="ml-3">
                  <div class="text-base/5 font-medium text-white">{{ state.currentUser.name }}</div>
                  <div class="text-sm font-medium text-gray-400">{{ state.currentUser.email }}</div>
                </div>
              ...
              <div class="mt-3 space-y-1 px-2">
                <DisclosureButton  
                v-for="item in userNavigation" :key="item.name" as="a" :href="item.href" @click="onClick(item.name)" class="block rounded-md px-3 py-2 text-base font-medium text-gray-400 hover:bg-gray-700 hover:text-white">{{ item.name }}</DisclosureButton>
              </div>
            </div>
          </DisclosurePanel>
        </Disclosure>
  ...
  const navigation = [
    ...
    { name: 'Users', href: '/account/users', current: false },
  ]
